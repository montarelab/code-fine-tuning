...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        if cur_iter % 500 == 0 and cfg.SOLVER.MAX_EPOCH < 12: # was 500 previously (also once it was 100)
            # save the model (always overwrite)
            index = cur_iter / 500 # was 500 previously
...
def train(cfg):
    ...
    if cfg.MODEL.FREEZE_TO >= 0:
        ...
        for name, p in model.named_parameters():
            ...
            layer += 1
        start_epoch = cu.load_train_checkpoint(cfg=cfg, model=model, optimizer=optimizer)
    else:
        start_epoch = cu.load_train_checkpoint(cfg, model, optimizer)
