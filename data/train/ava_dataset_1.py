...
class Ava(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        ] # extraction on entire video level, here _image_paths still contain all the frames, while boxes_and_labels
        # were already reduced by sampling rate
    ...
    def __getitem__(self, idx):
        ...
        # Score is not used.
        boxes = boxes[:, :4].copy() # don't see why 4 should be necessary, 2nd dimension should be 4 anyway
        ...
        metadata = [[video_idx, sec]] * len(boxes) # stretches list into one dimension
...
class Da(Ava):
    ...
    def __init__(self, cfg, split):
        ...
        self._seq_len = self._video_length * self._sample_rate
        self._num_classes = cfg.MODEL.NUM_CLASSES
        self._da_classes = cfg.DA.CLASSES
        # Augmentation params.
        self._data_mean = cfg.DATA.MEAN
        self._data_std = cfg.DATA.STD
        self._use_bgr = cfg.AVA.BGR
        self.random_horizontal_flip = cfg.DATA.RANDOM_FLIP
        if self._split == "da_train":
            self._crop_size = cfg.DATA.TRAIN_CROP_SIZE
            self._jitter_min_scale = cfg.DATA.TRAIN_JITTER_SCALES[0]
            self._jitter_max_scale = cfg.DATA.TRAIN_JITTER_SCALES[1]
            self._use_color_augmentation = cfg.AVA.TRAIN_USE_COLOR_AUGMENTATION
    ...
    def _load_data(self, cfg):
        ...
        (
            self._image_paths,
            self._video_idx_to_name,
        ) = da_helper.load_image_lists(cfg, is_train=(self._split == "da_train"))
        # Loading annotations for boxes and labels.
        boxes_and_labels = da_helper.load_boxes_and_labels(
            cfg, mode=self._split
        )
        ...
        (
            self._keyframe_indices,
            self._keyframe_boxes_and_labels,
        ) = da_helper.get_keyframe_data(boxes_and_labels)

        # Calculate the number of used boxes.
        self._num_boxes_used = da_helper.get_num_boxes_used(
            self._keyframe_indices, self._keyframe_boxes_and_labels
        )
    ...
    def _images_and_boxes_preprocessing_cv2(self, imgs, boxes):
        ...
        # Do color augmentation (after divided by 255.0).
        if self._split == "da_train" and self._use_color_augmentation:
            if not self._pca_jitter_only:
                imgs = cv2_transform.color_jitter_list(
                    imgs,
                    img_brightness=0.4,
    ...
    def __getitem__(self, idx):
        ...
        if self._split == "da_train" or "da_val":
            #print(imgs.shape)
            ...
            # Number of frames = 128, inter-clip spacing of 32 frames
            #imgs1= imgs[:,0:21,:,:]
            #imgs2= imgs[:,53:74,:,:]
            #imgs3= imgs[:,106:128,:,:]
            ...
            k = np.random.randint(low=0, high=self._da_classes)  #np.random.randint(low=0, high=self._da_classes)
            print(k)
            
            labels = np.zeros((boxes.shape[0], self._da_classes), dtype=int)
            
            
            # Shuffle images
            if k==0:
                imgs_shuf = torch.cat((imgs1, imgs2, imgs3), 1)
                labels[:,0] = np.ones((boxes.shape[0],), dtype=int)
                #print(imgs_shuf.shape)
                #print(labels.shape)
            if k==1:
                imgs_shuf = torch.cat((imgs1, imgs3, imgs2), 1)
                labels[:,1] = np.ones((boxes.shape[0],), dtype=int)
                #print(imgs_shuf.shape)
                #print(labels.shape)
            if k==2:
                imgs_shuf = torch.cat((imgs2, imgs1, imgs3), 1)
                labels[:,2] = np.ones((boxes.shape[0],), dtype=int)
                #print(imgs_shuf.shape)
                #print(labels.shape)
            if k==3:
                imgs_shuf = torch.cat((imgs2, imgs3, imgs1), 1)
                labels[:,3] = np.ones((boxes.shape[0],), dtype=int)
                #print(imgs_shuf.shape)
                #print(labels.shape)
            if k==4:
                imgs_shuf = torch.cat((imgs3, imgs1, imgs2), 1)
                labels[:,4] = np.ones((boxes.shape[0],), dtype=int)
                #print(imgs_shuf.shape)
                #print(labels.shape)
            if k==5:
                imgs_shuf = torch.cat((imgs3, imgs2, imgs1), 1)
                labels[:,5] = np.ones((boxes.shape[0],), dtype=int)
                #print(imgs_shuf.shape)
                #print(labels.shape)
