...
import slowfast.datasets.da_helper as da_helper
...
class AVAMeter(object):
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        elif (self.mode == "val") or (self.mode == "da_val"): # luca added da_val
            print('in log_iter_stats')
            stats = {
                "_type": "{}_iter".format(self.mode),
                "cur_epoch": "{}".format(cur_epoch + 1),
                "cur_iter": "{}".format(cur_iter + 1),
    ...
    def update_stats(self, preds, ori_boxes, metadata, loss=None, lr=None):
        ...
        if self.mode in ["val", "test", "da_val"]: #luca added da_val
    ...
    def finalize_metrics(self, log=True):
        ...
        if self.mode == "test" or (self.full_ava_test and ((self.mode == "val") or (self.mode == "da_val"))): #luca
            # added da_val
        ...
        if log:
            ...
            # The following lines are introduced to also have the ap per class logged
            """
            stats = {"mode": self.mode, "map": self.full_map["PascalBoxes_Precision/mAP@0.5IOU"]}
            
            stats = {"mode": self.mode,
                     "ap bend/bow (at the waist)": self.full_map["PascalBoxes_PerformanceByCategory/AP@0.5IOU/bend/bow (at the waist)"],
                     "ap crawl": self.full_map["PascalBoxes_PerformanceByCategory/AP@0.5IOU/crawl"],
                     "ap crouch/kneel": self.full_map["PascalBoxes_PerformanceByCategory/AP@0.5IOU/crouch/kneel"],
                     "ap dance": self.full_map["PascalBoxes_PerformanceByCategory/AP@0.5IOU/dance"],
                     "ap fall down": self.full_map["PascalBoxes_PerformanceByCategory/AP@0.5IOU/fall down"],
                     "map": self.full_map["PascalBoxes_Precision/mAP@0.5IOU"]}
            """
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        """
        if self.mode in ["val", "da_val", "test"]:
...
class DA_AVAMeter(AVAMeter):
    ...
    def __init__(self, overall_iters, cfg, mode):
        ...
        self.lr = None
        self.loss = ScalarMeter(cfg.LOG_PERIOD)
        self.full_ava_test = cfg.DA.FULL_TEST_ON_VAL
        self.mode = mode
        self.iter_timer = Timer()
        self.data_timer = Timer()
        self.net_timer = Timer()
        ...
        self.overall_iters = overall_iters
        self.excluded_keys = read_exclusions(
            os.path.join(cfg.DA.ANNOTATION_DIR, cfg.DA.EXCLUSION_FILE)
        )
        self.categories, self.class_whitelist = read_labelmap(
            os.path.join(cfg.DA.ANNOTATION_DIR, cfg.DA.LABEL_MAP_FILE)
        )



        gt_filename = os.path.join(
            cfg.DA.ANNOTATION_DIR, cfg.DA.GROUNDTRUTH_FILE
        )
        self.full_groundtruth = read_csv(gt_filename, self.class_whitelist)
        self.mini_groundtruth = get_ava_mini_groundtruth(self.full_groundtruth)

        _, self.video_idx_to_name = da_helper.load_image_lists(
            cfg, mode == "da_train"
        )
        self.output_dir = cfg.OUTPUT_DIR
