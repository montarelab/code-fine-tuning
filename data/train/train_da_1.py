...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        #inputs_backup = inputs[:]

        if cfg.DETECTION.ENABLE:
        
            preds, _  = model(inputs, meta["boxes"])
            #_,_, act_feats_remember= model2(inputs_backup, meta["boxes"])
        else:
            preds = model(inputs)
        ...
        #inputs2_backup = inputs2[:]
        _, preds2 = model(inputs2, meta2["boxes"])
        #_,_,aux_feats_remember= model2(inputs2_backup, meta2["boxes"])
        ...
        #feat_dist_lambda = 0.5
        #if feat_dist_lambda > 0:
        #    feat_dist = torch.dist(aux_feats, aux_feats_remember, p=2)
        #    feat_dist_loss = feat_dist_lambda * feat_dist
        #loss2 = loss2 + feat_dist_loss
        ...
        if cfg.DA.F_CONFUSION != -1 and cur_iter % cfg.DA.F_CONFUSION == 0:  # was 500 previously (also once it was 100)
            ...
            cm = misc.create_cm(y_true_interim, y_pred_interim, 6)
            misc.plot_confusion_matrix(cm=cm, normalize=False, target_names=['0', '1', '2', '3', '4', '5'],
    ...
    cm = misc.create_cm(y_true, y_pred, 6)
    misc.plot_confusion_matrix(cm=cm, normalize=False, target_names=['0', '1', '2', '3', '4', '5'],
...
def train_da(cfg):
    ...
    #optimizer_notouch= optim.construct_optimizer(model2, cfg)
    #optimizer_aux_notouch = optim.construct_optimizer(model2, cfg)

    # Load a checkpoint to resume training if applicable.
    #start_epoch = cu.load_train_checkpoint(cfg, model, optimizer)
    #_ = cu.load_train_checkpoint_da(cfg, model2, optimizer_notouch, optimizer_aux_notouch)
    ...
    if cfg.MODEL.FREEZE_TO >= 0:
        ...
        for name, p in model.named_parameters():
            ...
            layer += 1
        start_epoch = cu.load_train_checkpoint_da(cfg=cfg, model=model, optimizer=optimizer, optimizer_da=optimizer_aux)
    else:
        start_epoch = cu.load_train_checkpoint_da(cfg, model, optimizer, optimizer_aux)
    
    # activate this section if a detailed layer wise count shall be returned
    """
    counter = 0
    layer = 0
    for name, p in model.named_parameters():
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if is_checkp_epoch:
            ...
            cu.save_checkpoint_da(cfg.OUTPUT_DIR, model, optimizer, optimizer_aux, cur_epoch, cfg)
