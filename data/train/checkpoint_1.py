...
def save_checkpoint_da(path_to_job, model, optimizer, optimizer_da, epoch, cfg):
    """
    Save a checkpoint.
    Args:
        model (model): model to save the weight to the checkpoint.
    ...
    checkpoint = {
        ...
        "model_state": normalized_sd,
        "optimizer_state": optimizer.state_dict(),
        "optimizer_da_state": optimizer_da.state_dict(),
        "cfg": cfg.dump(),
    }
    # Write the checkpoint.
    path_to_checkpoint = get_path_to_checkpoint(path_to_job, epoch + 1)
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            ##############################################################
            # code inserted by Luca Sieber
            if last_pretrain != "":

                start_removing = False
                pre_train_dict_match_new = pre_train_dict_match.copy()
                layer = 0
                for k, v in pre_train_dict_match.items():
                    print(layer, k)
                    if start_removing:
                        del pre_train_dict_match_new[k]
                    if k == last_pretrain:
                        start_removing = True
                    layer += 1


                pre_train_dict_match = pre_train_dict_match_new.copy()
                layer = 0
                for k, v in pre_train_dict_match.items():
                    print(layer, k)
                    layer += 1
    ...
    if convert_from_caffe2:
        ...
        if "epoch" in checkpoint.keys() and not epoch_reset:
            ...
            if optimizer:
                optimizer.load_state_dict(checkpoint["optimizer_state"])
            if optimizer_da:
                optimizer_da.load_state_dict(checkpoint["optimizer_da_state"])
        else:
            epoch = -1
    return epoch
...
def load_train_checkpoint(cfg, model, optimizer):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            ...
            last_pretrain=cfg.MODEL.LAST_PRE_TRAIN,
        )
        start_epoch = checkpoint_epoch + 1
    else:
        start_epoch = 0
...
def load_train_checkpoint_da(cfg, model, optimizer, optimizer_da):
    """
    Loading checkpoint logic for training.
    """
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        logger.info("Load from given checkpoint file.")
        checkpoint_epoch = load_checkpoint_da(
            cfg.TRAIN.CHECKPOINT_FILE_PATH,
            model,
            cfg.NUM_GPUS > 1,
            optimizer,
