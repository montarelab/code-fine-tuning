...
class TaskInfo:
    def __init__(self):
        ...
        # Luca Sieber: following lines need to be activated in hotfix mode
        hotfix = False
        if hotfix:
            self.written = False
    ...
    def add_bboxes(self, bboxes):
        ...
        if hotfix:
            if not self.written:
                torch.save(bboxes, '/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/' + str(self.id) + '_boxes.pt')
                self.written = True
            if self.id == 0:
                torch.save(torch.tensor(0), '/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/counter.pt')
            print('bboxes added', self.id)
