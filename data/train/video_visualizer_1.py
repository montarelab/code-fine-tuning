...
class VideoVisualizer:
    ...
    def draw_one_frame(
        ...
        font_size = min(
            max(np.sqrt(frame.shape[0] * frame.shape[1]) // 35, 5), 18 # 9 modification by luca
        )
    ...
    def draw_clip(
        ...
        # Luca Sieber: decide whether hotfix mode or not
        hotfix = False

        if not hotfix:

            # Activate this piece of code, if boxes should fade
            ...
            # hotfix to avoid fading of the boxes, luca
            alpha_ls = np.concatenate([np.linspace(1, 1, num=half_left), np.linspace(1, 1, num=half_right), ])

            text_alpha = text_alpha
            frames = frames[repeated_seq]
            bboxes = bboxes.cuda()
            ######################################
            # Luca Sieber: the following lines need to be activated in hotfix mode
            # check on file system which the number of the current task
            task_id = torch.load('/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/counter.pt').item()
            print('printing the counter', str(task_id))
            print('half_left', half_left)
            print('half_right', half_right)
            # torch.load('/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/' + str(self.id) + '_boxes.pt')
            import os
            print('path exists:',
                  os.path.isfile('/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/' + str(task_id) + '_boxes.pt'))
            print('number of frames to be drawn', len(frames))

            # check whether it is the the last iteration and we don't need to adapt to a next task
            not_last = os.path.isfile('/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/' + str(task_id + 1) +
                                      '_boxes.pt')

            # load the bounding boxes of next task else set counter in file system to zero and remove all the boxes
            if not_last:
                bboxes_next = torch.load('/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/' + str(task_id + 1) +
                                      '_boxes.pt')
                bboxes_next = bboxes_next.cuda()
                # adapt order in bboxes_next such that the order matches bboxes
                if list(bboxes.size())[0] > 1:
                    dist_0 = torch.norm(bboxes_next[0, :] - bboxes[0, :], 2)
                    dist_1 = torch.norm(bboxes_next[1, :] - bboxes[0, :], 2)
                    if dist_1 < dist_0:
                        print('swapping', bboxes_next)
                        x = bboxes_next[0].clone()
                        bboxes_next[0] = bboxes_next[1]
                        bboxes_next[1] = x
                        print('swapping', bboxes_next)

                    print(dist_0)
                    print(dist_1)
                    print('yes the size is greater than 1')


            elif not not_last:
                bboxes_prev = torch.load('/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/' + str(task_id - 1) +
                                      '_boxes.pt')
                # adapt order in bboxes_prev such that the order matches bboxes

                ###############
                bboxes_prev = bboxes_prev.cuda()
                # adapt order in bboxes_next such that the order matches bboxes
                if list(bboxes.size())[0] > 1:
                    dist_0 = torch.norm(bboxes_prev[0, :] - bboxes[0, :], 2)
                    dist_1 = torch.norm(bboxes_prev[1, :] - bboxes[0, :], 2)
                    if dist_1 < dist_0:
                        print('swapping', bboxes_prev)
                        x = bboxes_prev[0].clone()
                        bboxes_prev[0] = bboxes_prev[1]
                        bboxes_prev[1] = x
                        print('swapping', bboxes_prev)
                ...
                #torch.save(torch.tensor(0), '/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/counter.pt')
                for i in range(task_id + 1):
                    os.remove('/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/' + str(i) +
                                      '_boxes.pt')
                    print('lol')

            # increase the task id by one
            torch.save(torch.tensor(task_id + 1), '/home/sieberl/SA2020/pyslowfast/slowfast/slowfast/hotfix/counter.pt')
            ...
            if task_id == 0:
                ...
                for alpha, frame in zip(alpha_ls, frames):
                    ...
                    if larl < 33:
                        helper = torch.lerp(bboxes, bboxes_next, - (1 - torch.full_like(bboxes, larl / 32)))
                        draw_img = self.draw_one_frame(frame, preds, helper, alpha=alpha, text_alpha=text_alpha,
                                                   ground_truth=ground_truth, )
                        larl += 1
                        if adjusted:
                            draw_img = draw_img.astype("float32") / 255
                        ...
                        # linearly interpolate boxes inbetween
                        helper = torch.lerp(bboxes, bboxes_next, torch.full_like(bboxes, (larl-32) / 32))
                        print('helper', helper)
                        print('bboxes', bboxes)
                        larl += 1
                ...
                for alpha, frame in zip(alpha_ls, frames):
                    ...
                    # linearly interpolate boxes inbetween
                    #print('origbboxes', bboxes, bboxes.is_cuda)
                    #helper[0,:] = torch.lerp(bboxes[0,:], bboxes_next[0,:], torch.full_like(bboxes[0,:], larl/32))
                    helper = torch.lerp(bboxes, bboxes_next, torch.full_like(bboxes, larl / 32))
                    #print('helper', helper, helper.is_cuda)
                    #print('bboxes_next', bboxes_next, bboxes_next.is_cuda)
                    larl += 1
                ...
                larl = 0
                for alpha, frame in zip(alpha_ls, frames):
                    helper = torch.lerp(bboxes, bboxes_prev, - (torch.full_like(bboxes, larl / 32)))
                    draw_img = self.draw_one_frame(frame, preds, helper, alpha=alpha, text_alpha=text_alpha,
                        ground_truth=ground_truth, )
                    larl += 1
                    if adjusted:
