...
def load_image_lists(cfg, is_train):
    ...
    """
    list_filenames = [
        os.path.join(cfg.DA.FRAME_LIST_DIR, filename)
        # in case there would be several files listing training samples (here only one)
        for filename in (
            cfg.DA.TRAIN_LISTS if is_train else cfg.DA.TEST_LISTS
        )
    ]
    image_paths = defaultdict(list) # seems to be a dictionary of lists
    video_name_to_idx = {}
    video_idx_to_name = []
        ...
        with PathManager.open(list_filename, "r") as f:
            ...
            for line in f:
                ...
                image_paths[data_key].append(
                    os.path.join(cfg.DA.FRAME_DIR, row[3])
                )
...
def load_boxes_and_labels(cfg, mode):
    ...
    if mode == "da_train":
        gt_lists = cfg.DA.TRAIN_GT_BOX_LISTS
    elif mode == "da_val":
        gt_lists = cfg.DA.TEST_GT_BOX_LISTS
    else:
        gt_lists = []


    #gt_lists = cfg.DA.TRAIN_GT_BOX_LISTS if mode == "da_train" else []
    #gt_lists = cfg.DA.TEST_GT_BOX_LISTS if mode == "da_val" else []

    pred_lists = (
        cfg.DA.TRAIN_PREDICT_BOX_LISTS
        if mode == "da_train"
        else cfg.DA.TEST_PREDICT_BOX_LISTS
    )
    ann_filenames = [
        os.path.join(cfg.DA.ANNOTATION_DIR, filename)
        for filename in gt_lists + pred_lists
    ]
    ann_is_gt_box = [True] * len(gt_lists) + [False] * len(pred_lists)

    detect_thresh = cfg.DA.DETECTION_SCORE_THRESH
    # Only select frame_sec % 4 = 0 samples for validation if not
    # set FULL_TEST_ON_VAL.
    boxes_sample_rate = (
        4 if mode == "da_val" and not cfg.DA.FULL_TEST_ON_VAL else 1
    )
...
def get_keyframe_data(boxes_and_labels):
    ...
    for video_idx in range(len(boxes_and_labels)):
        ...
        for sec in boxes_and_labels[video_idx].keys():
            if sec not in AVA_VALID_FRAMES: # hyperparameter to only parse after 900 frames in AVA video
                continue
...
def parse_bboxes_file(
    ...
    unique_box_count = 0
    luca = 0
    for filename, is_gt_box in zip(ann_filenames, ann_is_gt_box): # zip connects iterators
