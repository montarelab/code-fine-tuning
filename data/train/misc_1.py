...
def plot_confusion_matrix(cm,
    ...
    if cur_iter is not None:
        writer = SummaryWriter(log_dir=tensorboard + '/tb_confusion' + str(epoch) + '_' + str(cur_iter))
        writer.add_image('confusion_' + str(epoch) + '_' + str(cur_iter), img, 0)
    else:
        writer = SummaryWriter(log_dir=tensorboard + '/tb_confusion' + str(epoch))
        writer.add_image('confusion_' + str(epoch), img, 0)

    writer.close()



def create_cm(y_true, y_pred, cl):
    # initialize confusion matrix in terms of dimension
    cm = np.zeros((cl, cl), dtype=int)
